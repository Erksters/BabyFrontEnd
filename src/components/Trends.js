...
import { get_many_counts_by_year } from "../api/api"

import Loading from "./Loading/loading";
import LogTrends from "./Log/LogTrends";

const Trends = () => {
    const myurl = get_many_counts_by_year;

    const [data, setData] = useState([]);
    const [SubmittedData, setSubmittedData] = useState(false);
    const [username, setUsername] = useState('')
    const dialogue1 = `Type in a name to search for.`
    const dialogue2 = `The result will be a chart containing the number of children born with that name each year starting at 1880`
    ...
    return (
        <>
            <div className="m-3 " style={{ justifyContent: 'center', display: 'flex' }}>
                Take a look at the yearly trends for {username}
            </div>
